...
def generate_manifest(event, context):
    ...
    print(f"Event timestamp: {event['Records'][0]['eventTime']}")
    print(f'{str(key).split("/")[-2]} body_dict on read:{body_dict}')
    ...
    try:
        ...
        print(f'{plugin} body_dict before dump to body:{body_dict}')
